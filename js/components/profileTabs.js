...
export function updateActiveProfileUI() {
    ...
    // Centralizar clases activas en los botones de fase
    ...
    if (state.activeProfileId === 'calendar') {
        ...
        if (koSel) koSel.style.display = 'none'; // Nunca mostrar koSel en Calendario
    ...
    const claimOwnerName = document.getElementById('claim-profile-owner-name');
    const btnClaim = document.getElementById('btn-claim-profile-now');
    
    if (claimBanner) {
        const isActiveProfileParticipant = typeof state.activeProfileId === 'number';
        
        if (isActiveProfileParticipant) {
            const activeProfile = state.profiles.find(p => p.id === state.activeProfileId);
            
            if (state.myProfileId === null) {
                // El usuario no ha reclamado ningún perfil aún
                claimBanner.style.display = 'block';
                if (claimOwnerName) claimOwnerName.textContent = activeProfile ? activeProfile.name : '';
                if (btnClaim) {
                    btnClaim.style.display = 'inline-block';
                    btnClaim.textContent = '👉 Este soy yo';
                }
            } else if (state.activeProfileId !== state.myProfileId) {
                // El usuario ya reclamó otro perfil
                claimBanner.style.display = 'block';
                if (claimOwnerName) claimOwnerName.textContent = activeProfile ? activeProfile.name : '';
                if (btnClaim) {
